...
const mirrorFamilies = [
  {
    ...
    href: "/products/optical-mirrors/spherical-mirrors",
  },
  {
    number: "02",
    name: "Flat Mirrors",
    ...
    href: "/products/optical-mirrors/flat-mirrors",
  },
];
  ...
  return (
    <main>
          ...
          <div className="lens-family-grid">
            {mirrorFamilies.map((mirror, index) => (
              <article className="lens-family-card" id={`item-${index + 1}`} key={mirror.name}>
                <a className="family-card-hit-area" href={mirror.href} aria-label={`View ${mirror.name} specifications`} />
                <div className="lens-photo"><a href={mirror.href}><img src={mirror.image} alt={mirror.name} /></a></div>
                <div className="lens-family-copy">
                  <span className="product-number">{mirror.number}</span>
                  <h3><a href={mirror.href}>{mirror.name}</a></h3>
      ...
      <section className="lens-cta section">
        ...
        <div><p>Send us your substrate, dimensions, surface figure, wavelength range, angle of incidence and coating requirements.</p><a className="button" href="mailto:sales@baitecoptics.com">sales@baitecoptics.com <ArrowUpRight /></a></div>
      </section>
